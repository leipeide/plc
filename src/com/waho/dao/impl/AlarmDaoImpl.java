...
public class AlarmDaoImpl implements AlarmDao {
	...
	public int insert(Alarm alarm) throws Exception {
		...
		return qr.update("INSERT INTO alarm (`date`, `deviceMac`, `nodeAddr`, `type`,userid) VALUES (?, ?, ?,?,?)",
				alarm.getDate(), alarm.getDeviceMac(), alarm.getNodeAddr(),alarm.getType(),alarm.getUserid());
		
	}

	@Override
	public List<Alarm> selectByDeviceMacAndNodeAddrAndType(String deviceMac, String nodeAddr, int alarmDisconnect)
			throws Exception {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
		return qr.query("select * from alarm where deviceMac=? and nodeAddr=? and type=?",
				new BeanListHandler<Alarm>(Alarm.class), deviceMac, nodeAddr, alarmDisconnect);
	}
	...
	@Override
	public List<Alarm> selectByDeviceMacAndType(String deviceMac, int alarmDisconnect) throws Exception {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
		return qr.query("select * from alarm where deviceMac=? and type=?",
				new BeanListHandler<Alarm>(Alarm.class), deviceMac, alarmDisconnect);
	}
