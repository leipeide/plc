...
import com.waho.socket.util.CmdAddNodeHandler;
import com.waho.socket.util.CmdBroadcastWriteStateHandler;
import com.waho.socket.util.CmdDelNodeHandler;
...
public class SocketOperate extends Thread {
	 ...
	 * 添加节点指令处理
	 */
	private static CmdAddNodeHandler CANH;
	/**
	 * 删除指令处理
	 */
    private static CmdDelNodeHandler CDNH;
	/**
	...
	static {
		...
		CDNH = CmdDelNodeHandler.getInstance(CUH); // 处理删除节点回复指令
		CANH = CmdAddNodeHandler.getInstance(CDNH);// 处理增加节点回复指令
		CRNSH = CmdReadNodeStateHandler.getInstance(CANH);// 处理读节点信息回复指令
	...
	public void run() {
		...
		try {
			out = socket.getOutputStream();//服务器端发送给客户端的数据流。
			this.readThread = new Thread() {  //读线程
				public void run() {
					byte[] temp = new byte[1024];
					int length = 0;
					InputStream in = null; //客户端（集控器）发送给服务器端的数据流。
					try {
						in = socket.getInputStream();
						while (!socket.isClosed()) {
							// 读取客户端发送的信息
							if ((length = in.read(temp)) != -1) { //当in.read()值等于-1时，代表数据读取完毕
								timeCount = 0;
								// 添加数据粘包处理
