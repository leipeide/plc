...
public class DeviceDaoImpl implements DeviceDao {
	...
	public void checkNodesRegister(Device device) throws Exception {
		if (device.getRegistDate() != null && device.getRegisterMinutes() != 0) { 
			...
			if (check > 0 && check <= device.getRegisterMinutes() * 60 * 1000) { //判断是否在允许节点主动注册时间内
	...
	@Override
	public Device selectDeviceByDeviceName(String deviceName) throws Exception {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
		Device device = qr.query("select * from devices where deviceName=?", new BeanHandler<Device>(Device.class), deviceName);
		checkNodesRegister(device);
		return device;
	}
	...
	public int insert(Device device) throws Exception {
		...
		/*return qr.update(
			...
			*/
		return qr.update(
				"INSERT INTO devices (`deviceMac`, `userid`, `online`, `currentNodes`, `maxNodes`, `deviceName`) VALUES (?, ?, ?, ?, ?, ?)",
				device.getDeviceMac(), device.getUserid(), device.isOnline(), device.getCurrentNodes(),
				device.getMaxNodes(), device.getDeviceName());
	}
	...
	public int updateDeviceRegister(String deviceMac, Date date, int minutes) throws Exception {
		...
		long check = now.getTime() - date.getTime(); //毫秒差
	...
	@Override
	public int updateDeviceuseridByDeviceMac(Device device) throws Exception {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
	     return qr.update("UPDATE devices SET userid=? WHERE deviceMac=?",device.getUserid(),device.getDeviceMac());
	}

	@Override
	public int updateDeviceuseridByDeviceName(Device device) throws Exception {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
		return qr.update("UPDATE devices SET userid=? WHERE deviceName=?",1000,device.getDeviceName());
	}

	@Override
	public int updateDeviceNameByDeviceId(Device device) throws Exception {
		QueryRunner qr = new QueryRunner(C3P0Utils.getDataSource());
		return qr.update("UPDATE devices SET deviceName=? WHERE id=?",device.getDeviceName(),device.getId());
	}
