...
public class ReadNodeNumberState implements SocketState {
	...
	public SocketState clientDataHandle(byte[] data, int length, SocketDataHandler handler, Device device,
		...
		} else if (sc != null && (byte) sc.getCommand() == SocketCommand.CMD_READ_MAC) {
	...
	private void dataHandler(SocketCommand sc, Device device) {
		...
		num[0] = sc.getData()[1];
		num[1] = sc.getData()[0];
		int num_int = Integer.parseInt(SocketCommand.parseBytesToHexString(num, num.length), 16);

		byte[] max = new byte[2];
		max[0] = sc.getData()[3];
		max[1] = sc.getData()[2];
		int mx_int = Integer.parseInt(SocketCommand.parseBytesToHexString(max, max.length), 16);
