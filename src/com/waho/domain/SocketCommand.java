...
public class SocketCommand {
	 ...
	 * 删除从节点的指令码
	 */
	public static final byte CMD_DELETE_NODE = 0x0C;
    /**
     * 手动添加从节点指令码
     */
	public static final byte CMD_ADD_NODE = 0x0D;
	/**
	 * 心跳包指令码
	 */
	 ...
	 * 删除从节点回复指令码
	 */
	public static final byte CMD_DELETE_NODE_REP =(byte) 0x8C;
	/**
	 * 添加从节点回复指令
	 */
	public static final byte CMD_ADD_NODE_REP = (byte)0x8D;
	/**
	 * 未知指令码
	 */
	 ...
	 * 通讯协议类型
	 */
	private static final byte PROTOCOL_TYPE =0x01;  
	/**
	 * 信息域生产器
	 */
	...
	public static byte[] GenerateWriteNodeStateCommandData(String nodeAddr, String light1State, String light2State,
		...
		//判断百分数是否大于100或小于0
		int light1PowerPercent1 = Integer.parseInt(light1PowerPercent);
		if(light1PowerPercent1 > 100) { 
			light1PowerPercent1 = 100;
		...
		//判断百分数是否大于100或小于0
				int light2PowerPercent2 = Integer.parseInt(light2PowerPercent);
				if(light2PowerPercent2 > 100) { 
					light2PowerPercent2 = 100;
	...
	public static byte[] GenerateBroadcastWriteStateCommandData(String light1State, String light2State,
			String light1PowerPercent, String light2PowerPercent) {
		byte[] bytes = new byte[10];
		System.arraycopy(BROADCAST_ADDR, 0, bytes, 0, 6);
		if ("on".equals(light1State)) {
			bytes[6] = LIGHT_STATE_ON;
		} else {
			bytes[6] = LIGHT_STATE_OFF;
	 ...
	 * @return
	 */
	public static byte[] GenerateBroadcastTimingWriteStateCommandData(boolean light1State, boolean light2State,
			String light1PowerPercent, String light2PowerPercent) {
		byte[] bytes = new byte[10];
		System.arraycopy(BROADCAST_ADDR, 0, bytes, 0, 6);
		if (light1State) {
	 ...
	 * @return
	 */
    public static byte[] GenerateAddNodesCommandDate(int num, String[] nodeAddrString) {
    	int len = 7 * num + 1;
    	...
    	for(int i = 0 ; i < num ; i++) {
			String nodeAddr = nodeAddrString[i];
			byte[] addr = SocketCommand.parseHexStringToBytes(nodeAddr);
			System.arraycopy(addr, 0, bytes, i*7+1, 6);
			bytes[(i+1)*7] = PROTOCOL_TYPE;
			}
     ...
     * @return
     */
    public static byte[] GenerateDelNodesCommandDate(int num, String[] nodeAddr) {
    	int len = 6 * num + 1;
    	byte[] bytes = new byte[len];
    	if(num < 0x0f) {
    		bytes[0] = (byte)num;
    	...
    	for(int i = 0; i < num; i++) {
    		byte[] addr = SocketCommand.parseHexStringToBytes(nodeAddr[i]);
    		System.arraycopy(addr, 0, bytes, i*6+1, 6);	
    	}
    	//System.out.println("数组:"+Arrays.toString(bytes));
    	return bytes;
    }
	/**
	 * 将二进制的指令封装成SocketCommand对象（初步判断集控器发送的指令处理）
	 * @param bytes 二进制指令
	 * @param len 指令长度
	...
	public static SocketCommand parseSocketCommand(byte[] bytes, int len) {
		...
		if (bytes[0] == HEADER && bytes[len - 1] == TAIL && len == temp) {// 头尾正确&&长度正确
			...
			//System.out.println("测试指令："+SocketCommand.parseBytesToHexString(sc.tobyteArray(),sc.getLen()));
