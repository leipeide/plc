  ...
  %>
	<script type="text/javascript">
      		layui.use('layer', function() {
				var layer = layui.layer;
 				layer.msg('<%=message%>',{
 					  //time: 4000 //（如果不配置，默认是3秒）
 					}, function(){ 
 					});   
			});
      </script>
	<%} %>
	<form id="nodeform" class="layui-form"
		action="${pageContext.request.contextPath }/nodeSerachServlet?deviceid=${deviceid}&currentPage=${pb.currentPage}"
		method="post">
		<div class="nav">
			...
			<div class="div-serach">
				<input type="text" name="serach" placeholder="请输入节点地址"
					style="width: 150px; height: 32px" class="layui-input">
			</div>
			<div class="div-submit">
 				<input type="submit" name="submit" value="搜索" style="width: 45px; height: 30px">
			</div>
		</div>
	...
	<div class="pagination" style="text-align: center">
		...
		<c:if test="${pb.totalPage == '0'}">
				...
				<button id="pageBt" class="layui-btn layui-btn-sm"
		...
		req.onreadystatechange = function() {
			if (req.readyState == 4) {//请求成功
				if (req.status == 200) {// 服务器响应成功
					//获取节点页面信息PBS，pbs与当前界面的pb是两个不同的变量，两者名称不可一致
					...
					for (var i = 0; i < nodes.length; i++) {
						node = nodes[i];
						inner = inner + "<tr><td>" + node.nodeAddr + "&nbsp;&nbsp;<a href='${pageContext.request.contextPath }/nodeMessageChartServlet?nodeAddr="+ node.nodeAddr +"'><i class='layui-icon layui-icon-chart'></i></a></td>\
						<td><a href='javascript:;'onclick=\"reName('${pageContext.request.contextPath }/nodeRenameFormServlet'," + node.id + ")\">" +node.nodeName +"</a></td>\
							<td>";
							...
							<td>" + node.power + " W</td>\
							<td><a href='javascript:;' onclick=\"nodeControl('${pageContext.request.contextPath }/nodeFormServlet', " + node.id 

+"," + node.light1State + "," + node.light1PowerPercent + "," + node.light2State + "," + node.light2PowerPercent + ")\">单灯控制</a></td>\
							<td><a href='javascript:;' onclick=\"nodeRefresh('${pageContext.request.contextPath }/nodeRefreshServlet?nodeid=" + node.id 
...
+ "')\">刷新</a></td>\
	...
	//节点页面加载完成后进行AJAXRequest()函数
	window.onload = function() {
		AJAXRequest();
	}
	setInterval(AJAXRequest,1000*3); //3  秒刷新一次
	</script>
</body>
</html>